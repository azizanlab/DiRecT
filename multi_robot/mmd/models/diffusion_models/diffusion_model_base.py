...
class GaussianDiffusionModel(nn.Module, ABC):
    ...
    def p_sample_loop(self, shape,
        ...
        timeit = sample_kwargs.get('timeit', False)
        if timeit: 
            dict_add = lambda _d1, _d2: {k: _d1.get(k, 0) + _d2.get(k, 0) for k in (set(_d1.keys()) | set(_d2.keys()))}
            total_time_elapsed = {} # 'diffuse', 'coupling', 'project'
        for i in reversed(range(-n_diffusion_steps_without_noise, n_diffusion_steps)):
            t = make_timesteps(batch_size, i, device)
            if timeit:
                x, values, time_elapsed_iter = sample_fn(self, x, hard_conds, context, t, **sample_kwargs)
                total_time_elapsed = dict_add(total_time_elapsed, time_elapsed_iter)
            else: 
        ...
        if return_chain:
            ...
            if timeit:
                return x, chain, total_time_elapsed
            else:
                return x, chain
        
        if timeit:
            return x, total_time_elapsed
        return x
    ...
    def run_inference(self, context=None, hard_conds=None, n_samples=1, return_chain=False, **diffusion_kwargs):
        ...
        for k, v in hard_conds.items():
            ...
            if isinstance(v, torch.Tensor):
                new_state = v.unsqueeze(0).expand(n_samples, *v.shape)
            else:
                new_state = v
        ...
        timeit = diffusion_kwargs.get('timeit', False)
        if timeit:
            samples, chain, time_elapsed = self.conditional_sample(
                hard_conds, n_diffusion_steps=self.n_diffusion_steps, context=context, batch_size=n_samples, 
                return_chain=True, **diffusion_kwargs
            )
        else:
        ...
        if return_chain:
            if timeit:
                return trajs_chain_normalized, time_elapsed
            return trajs_chain_normalized

        # return the last denoising step
        if timeit:
            return trajs_chain_normalized[-1], time_elapsed
