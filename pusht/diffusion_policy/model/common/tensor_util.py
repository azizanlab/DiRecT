...
def op_tensor(x, y, op, **op_kwargs):
    """
    Apply operation @op to torch.Tensor objects in two dictionaries. 

    Args:
        x (dict or list or tuple): a possibly nested dictionary or list or tuple
        y (dict or list or tuple): a possibly nested dictionary or list or tuple
        op (function): operation to apply to each pair of tensors

    Returns:
        z (dict or list or tuple): new dict
    """
    z = {}
    for (kx, vx), (ky, vy) in zip(flatten_nested_dict_list(x), flatten_nested_dict_list(y)):
        assert kx == ky
        assert isinstance(vx, torch.Tensor)
        assert isinstance(vy, torch.Tensor)
        z[kx] = op(vx, vy, **op_kwargs)
