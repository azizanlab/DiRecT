#!/bin/bash
#SBATCH --job-name=sweep_direct_pgd_scl
...
T_START_GUIDE=16
T_START_PROJ=16
EXP_NAME="sweep_direct_pgd_scl2"
...
echo "============================================"
echo "DiRecT-PGD scl Sweep - $(date)"
...
for SCL in "${SCL_VALUES_LB[@]}"; do
    COMMENT="${EXP_NAME}-DiRecT_LB-scl${SCL}"
    echo ">>> DiRecT-LB scl=${SCL}"
    $CMD \
        --guider ${GUIDER} \
        --projector ${PROJECTOR} \
        --cost-func-key ${COST} \
...
for SCL in "${SCL_VALUES_DPP[@]}"; do
    COMMENT="${EXP_NAME}-DiRecT_DPP-scl${SCL}"
    echo ">>> DiRecT-DPP scl=${SCL}"
    $CMD \
        --guider ${GUIDER} \
        --projector ${PROJECTOR} \
        --cost-func-key ${COST} \
...
echo "============================================"
echo "DiRecT-PGD scl Sweep complete - $(date)"
echo "============================================"
