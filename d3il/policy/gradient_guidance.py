"""
GradientGuidancePolicy: DPS-style posterior sampling during DDPM reverse process.

At each denoising step, performs gradient ascent on a value function evaluated
on the predicted x_0 (Tweedie estimate), then takes the standard DDPM step.
...
class GradientGuidancePolicy(nn.Module):
    """DDPM sampling with DPS gradient guidance.

    Reverse-diffuses from t=1 to t=0, applying gradient guidance on the
    predicted x_0 at each step where t <= gradient_start.
    """
    ...
    def __call__(self, conditions, batch_size=1):
        """Sample trajectories with gradient guidance.
