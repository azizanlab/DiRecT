...
def get_preprocess_fn(fn_names: Sequence[str], env: Any) -> Callable:
    fns = [eval(name)(env) for name in fn_names]
    return compose(*fns)

def get_policy_preprocess_fn(fn_names: Sequence[str]) -> Callable:
    fns = [eval(name) for name in fn_names]
    return compose(*fns)

def arctanh_actions(*args, **kwargs) -> Callable:
    epsilon = 1e-4
...
def add_deltas(env: Any) -> Callable:
    del env
...
def maze2d_set_terminals(env: Any) -> Callable:
